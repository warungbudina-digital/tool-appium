...
### 26b. Panel Lynx sisa RN7 1080x2340: Musik + Transisi + Overlay (2026-08-10, MOD EN)
**MUSIK — entry (deselect klip dulu; JANGAN BACK utk deselect, tap area kosong → BACK 1x tanpa seleksi = KELUAR editor+autosave draft):** `editor_track_music_add`@461,1560 → bottom-sheet **"Insert"** (Lynx): **Music@187,2018 · Effect@538,2018 · Record@889,2018 · X-close@538,2203** (X-Music identik Infinix 188). Music → **`MusicManageActivity`** (native): tab **Music@180,347 · Favorite@540,347 · Yours@900,347** + search y~500.
**⚠️ KATALOG CLOUD MATI DI MOD:** tab Music/kategori → **"Loading failed, tap to retry"** (server-side gated; MOD tak login → beda Infinix yg login Pro). **Jalur musik yg JALAN = tab "Yours" (impor lokal):** `Extract from Video`@287,679 · `File App`@792,679 (SAF) · baris Music Library@y889 · My Music@y1068 · New@y1248. Import → **"Import Tips"** sheet (ToU) → OK `btConfirm`@540,2137. Extract picker: `checkView`@90,2018 · `btConfirm`@540,2137 (JEBAKAN: album "All" bisa kosong/quirk). **Beat-sync reproduksi di MOD → pakai audio impor** (extract dari video sumber / file), lalu panel **Edit Beats/Auto Beat (client-side, JALAN)** — **DITUNDA capture sampai ada file audio** (butuh track ter-add dulu).
**TRANSISI (timeline=kanvas, tanpa selector):** pilih/di-seek dekat batas klip → ikon **"+" di junction antar-klip** (mis. @944,1890 utk junction clip2|clip3; posisi ikut zoom/scroll) → panel transisi: tab **Base@413,1752 / Matte@662,1752** → tipe (row y≈1896, scroll): **None@294 · Black@462 · White@630 · Zoom 1@796 · Zoom 2 · Dissolve 1 …** → **Duration slider** (row y≈2069, nilai "0.0s" kanan) → footer **X@164,2196 · "Apply to all"@538,2196 · ✓@916,2196**. (Reproduksi: mayoritas cut = None/hard-cut; transisi hanya bila IR tandai fade.)
**OVERLAY:** `editor_toolbar_toPiP` (content-desc/selector, saat scroll toolbar @~1000,2182) = pindah klip terpilih ke **trek overlay/PiP** (INSTAN, tanpa panel, §8c) · trek sticker/overlay = `editor_track_sticker_add`@461,1760 ("Tap to add sticker / Overlay"). 
**BONUS — tool toolbar late RN7 MOD (semua content-desc/selector, portabel):** …clipZoom · **mosaic**@ · **magnifier** · **story** · **reverse** · **freezeFrame** · **autoCaption** · **toPiP**. **CATATAN: `editor_toolbar_freezeFrame` ADA di build/MOD ini** (koreksi atas catatan lama Infinix "freeze frame tak ada di 2.17.0" — kemungkinan gated per akun/build); `reverse` & `autoCaption` juga ada (autoCaption kemungkinan server-gated spt katalog musik — verifikasi saat dipakai).

### 26c. Music Beats panel + Edit-Beats path RN7 1080x2340 (2026-08-10, MOD EN) — primitif terakhir
**Prasyarat MOD (katalog cloud mati):** impor audio via **Extract from Video**: music_add@461,1560 → Insert Music@187,2018 → MusicManageActivity → Yours@900,347 → Extract from Video@287,679 → Import Tips OK@540,2137 → picker klip (thumbnail col1@178,514) → tap klip → **`ExtractAudioActivity`** (trim; footer X@173 · pause · replay · ✓@904,2105) → ✓ → audio masuk trek musik. **JEBAKAN: coach-mark "Snap Setting Tip" muncul setelah add → "Got it"@538,2161.**
**Buka Edit Beats:** tap trek musik (item purple di baris `editor_track_music`) → strip aksi atas muncul: **Replace@193 · Beats@307 · Keyframe@421 · Options@538 · Lock@655 · Duplicate@770 · Delete@889** (y≈1433) + toolbar bawah music-clip (Volume@230 · Fade@383 · Split@535 · Denoise@687 · Voice Effect@839 · Auto Captions@991, y≈2170). **Beats@307,1433** → panel **"Music Beats"** (Lynx).
**Panel Music Beats:** judul@538,1383 · speed **0.5x@889 / 1x@975** (y1385) · waveform/metronome track y≈1580 (ruler frame 10f/20f) · **Add-Beat (bendera merah)@538,1913** (putar → ketuk seirama; 0.5x utk presisi) · footer **X(cancel)@164 · reset/undo@415 · play@665 · ✓(done)@916** (y2196). **⚠️ Tak ada toggle "Auto Beat" di build MOD ini** (Infinix §22 punya `swAutoBeat`) → beat MANUAL. Reproduksi beat-sync di MOD: ketuk Add-Beat di timestamp IR (bpm/beat), ATAU Auto Beat mungkin server-gated/absen — verifikasi bila perlu.

### 26d. ROBUSTNESS RN7 (jebakan + pola verify/recover, 2026-08-10)
Pola robust Infinix (§23f/§25 per-segmen run terpisah, verify seleksi via `ivDuplicate`, seek feedback-loop, guard panel) **BERLAKU di RN7** dgn koordinat §26. Tambahan spesifik RN7/MOD:
- **BACK di editor tanpa seleksi = KELUAR editor + autosave draft** (ke MainActivity "Projects"). Deselect WAJIB via **tap area timeline kosong** (mis. @850,1890), JANGAN BACK. Buka draft lagi: MainActivity Projects tab@134,401 → kartu proyek.
- **Coach-mark menelan input**: "Snap Setting Tip"→"Got it"@538,2161; "Snap ke Objek Terdekat"→Got it. Cek & tutup sebelum lanjut.
- **Transfer file laptop→akses-vps (ssh base64) WAJIB `| tr -d '\r\n'` sebelum `base64 -d`** — tanpa itu file 0-byte senyap → picker "No media yet" (verifikasi durasi via `content query .../video/media --projection _display_name:duration:_size`).
...
- **Bottom-nav sheet**: `Elements` | `Imports` (galeri/impor sendiri) | `Favorites` (bintang). Search bar di atas.
- Tab **Graphics** (CLIENT-SIDE, instan tanpa spinner): kategori **Shapes (36)** (lingkaran/kotak/segitiga/bintang…) · **Lines & Frames (323)** (panah/garis/dashed…) · **Social Media Logo (77)** (IG/YT/FB/TikTok) · **Social (275)** (tombol Subscribe/Like/Sub — SEBAGIAN ber-crown 👑 = Pro-gated). Tiap kategori punya "View More".
...
- **⚠️ Jebakan seleksi-klip (blind coord):** tap di main-track y≈1900 MALAH buka panel lain: `Cover`-box (x≈250 → editor Cover: tab **Video Frame·Library**, filmstrip "Drag to choose cover", **Cover Template**·**Edit with Flow Studio**) atau `editor_track_main_add` "+" (x≈424-600 → **media picker**: tab All▾·Stocks·Subtitle + All·Video·Photo, klip "Imported 00.08", "Selected footage:N/Next"). **Toolbar klip (`editor_toolbar_*`) TAK muncul** dari tap-tebak. → Sesi lanjut: pilih klip pakai helper **`repro-segments.js` tapClip** (pilih `timeline_item` DI BAWAH PLAYHEAD, commit `c10fa69`) atau dump `rvTimeline`/`editor_track_main` cari bounds anak-klip presisi, JANGAN tebak koordinat.
- **Kendala:** RN7 **di-baterai (49-51%, discharging)** = risiko Doze/drop (§27f); sesi enumerasi FX/Filter/Transisi/Keyframe **sebaiknya saat RN7 TERCOLOK CHARGER** (stabil + waktu cukup). Mitigasi sesi ini: `screen_off_timeout=1800000`.
